...
class SingleFactor():
    def __init__(self,para):
        ...
        Factor = Factor.loc[para.startDate:para.endDate,:]
    ...
    def every_month(self):
        ...
        for i,currentDate in enumerate(tqdm(self.tradingDateList[:-2])):
            ...
            dataFrame['premium'] = dataFrame['RET'] - dataFrame['Bechmark']

            if para.ret_style == '超额收益':
                dataFrame['Score'].type = np.str
                if para.weightMethod == '简单加权':
                    meanlist.append(np.array(dataFrame.groupby('Score')['premium'].mean()))
                    ...
                    for groupi in range(0,para.groupnum):
                        ...
                        dataFrame_['RET']=dataFrame_['premium'].copy()
